...
#define CUSTOM_SAMPLE_RATE  48000
...
{
    ESP_LOGI(TAG, "Audio size %d bytes, played at frequency %d Hz asynchronously", data_size, CUSTOM_SAMPLE_RATE);
    uint32_t cnt = 1;
    while (1) {
...
#if CONFIG_EXAMPLE_DAC_WRITE_ASYNC
    ESP_ERROR_CHECK(dac_continuous_start_async_writing(dac_handle));c:\Users\choua\esp\v5.2.1\esp-idf\examples\peripherals\dac\dac_continuous\dac_audio\tools\audio_example_file.h
